...
K MemCache<K, V>::get(K key) {
    if (exists(key)){
        MapItem<K, V> map_item = bykey.at(key);
...
void MemCache<K, V>::update_frequency_of_the(K key){
    ...
    FrequencyNode<K> *cur_freq = map_item.parent;
    FrequencyNode<K> *new_freq = cur_freq->next;
...
void MemCache<K, V>::put(K key, V value, unsigned long ttl) {
    ...
    FrequencyNode<K> *freq_node = HEAD->next;
    ...
    bykey.insert(make_pair(key, MapItem<K, V>(value, freq_node, key_node)));
    ++this->curr_size;
}
...
void MemCache<K, V>::apply_eviction_policy() {
    ...
    int remove_key;
    FrequencyNode<K> *LFUNode = HEAD->next;
...
template<typename K, typename V>
FrequencyNode<K>* MemCache<K, V>::get_new_frequency_node(
    int freq, 
    FrequencyNode<K>* prev, 
    FrequencyNode<K>* next
) {
    FrequencyNode<K> *new_freq_node = new FrequencyNode<K>();
...
void MemCache<K, V>::resize(size_t new_capacity) {
    ...
    size_t required_ram = available_ram * sizeof(MapItem<K, V>);
